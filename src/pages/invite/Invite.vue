    ...
    <div class="invite" v-if="0">
      <div class="header">
        <div class="invite-title">
          ...
          <a class="copy-button" v-clipboard:copy="myInfo.inviteLink" v-clipboard:success="onCopy"
            v-clipboard:error="onError" href="javascript:;">{{$t('invite.copy')}}</a>
        </div>
      ...
      <div class="main">
        <div class="invite-content">
          ...
          <div class="content">
            <p style="margin-bottom:20px;border-left: 5px solid #e7e7e7;padding-left: 25px;">
              {{$t('invite.ruleprofile1')}}<b>{{$t('invite.ruleprofile2')}}</b><b>{{$t('invite.ruleprofile3')}}</b>{{$t('invite.ruleprofile4')}}
            </p>
            ...
            <div
              style="font-size: 12px;padding: 10px;margin-left: 30px;width: 90%;margin-bottom: 10px;border: 1px solid #c8c8c8;">
            ...
            <p class="rule-update">{{$t('invite.lastupdate')}}：2022/01/17 12:15:34</p>
          </div>
        </div>
        ...
        <!--        <img src="https://binaricx.oss-ap-southeast-1.aliyuncs.com/2019/promotioncard.png"></img>-->
        ...
        <!--        <img src="https://binaricx.oss-ap-southeast-1.aliyuncs.com/invitebg.jpg"></img>-->
      ...
      <Drawer :title="$t('invite.pt_title')" :closable="false" v-model="showCardModal" width="600">
        <div class="ptcard-header">
          <img class="card-img" src="https://binaricx.oss-ap-southeast-1.aliyuncs.com/2019/promotioncard.png"></img>
          <div class="desc">
            ...
            <Button v-if="isLogin && !hasCard" type="error" size="large"
              @click="getFreeCard">{{$t('invite.pt_card_btn')}}</Button>
            <Button v-if="isLogin && hasCard" type="error" size="large"
              @click="usePromotionImage">{{$t('invite.usepromotion')}}</Button>
        ...
        <div class="ptcard-info">
          ...
          <p class="detail">{{$t('invite.pt_card_rule1')}}：support@bitnexc.com</p>
      ...
      <Drawer :title="promotionTitle" :closable="false" v-model="showPromotionModal" width="350"
        ...
        <Button type="error" size="large" :loading="saveImageLoading" long style="margin-top: 20px;"
          @click="saveImage">{{$t('invite.saveimage')}}</Button>
      </Drawer>
      ...
      <Drawer title="邀请图片" :closable="false" v-model="showInviteImageModal" width="350" style="text-align:center;">
        ...
        <p style="text-align:center;font-size:12px;color:#888;margin-top: 10px;">{{$t('invite.imagetips')}}</p>
        <Button type="error" size="large" :loading="saveImageLoading" long style="margin-top: 20px;"
          @click="saveInviteImage">{{$t('invite.saveimage')}}</Button>
      </Drawer>
    </div>
    <div class="px-3  ws-filter-tab d-flex flex-column mb-2" style="padding-top:10px; padding-bottom:10px;">
      <div class="d-flex justify-content-between align-items-center">
        <span class="ps-1 fs-8 d-flex align-items-center" style="border-left: 3px solid orange; line-height: 14px ;">
          {{$t('appmain.Myinvitation')}}
        </span>
        <span class="ps-1 fs-9 text-warning" style="opacity: .9; cursor: pointer;" v-clipboard:copy="myInfo.inviteLink"
          v-clipboard:success="onCopy" v-clipboard:error="onError">{{$t('appmain.Invitaionlink')}}</span>
      </div>
      <div class="px-4 pt-3 d-flex justify-content-between align-items-center">
        <span class="d-flex flex-column align-items-center">
          <span class="fs-7">{{myInfo.levelone}}</span>
          <span class="fs-9 pt-1" style="opacity: .6;">Lv1</span>
        </span>
        <span class="d-flex flex-column align-items-center">
          <span class="fs-7">{{myInfo.leveltwo}}</span>
          <span class="fs-9 pt-1" style="opacity: .6;">Lv2</span>
        </span>
        <span class="d-flex flex-column align-items-center">
          <span class="fs-7">{{myInfo.commission.toFixed(1)}}</span>
          <span class="fs-9 pt-1" style="opacity: .6;">{{$t('appmain.income')}}</span>
        </span>
        <span class="d-flex flex-column align-items-center">
          <span class="fs-7">{{myInfo.extrareward}}</span>
          <span class="fs-9 pt-1" style="opacity: .6;">{{$t('appmain.Rewards')}}</span>
        </span>
      </div>
    </div>
  </div>
    ...
    data() {
      ...
      return {
        ...
        myInfo: {
          ...
          extrareward: 0,
          partnerlevel: "-",
          inviteLink: "https://www.bitnexc.com/register?code=000000"
        },
        promotionRecordPage: {
          pageNo: 1,
          pageSize: 10
