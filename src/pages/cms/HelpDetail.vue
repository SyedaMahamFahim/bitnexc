  ...
  <div>
    ...
    <div class="helpdetail">
      <div class="route-wrap">
        ...
        <span>></span>
        <router-link :to="{path:'helplist',query:{cate:cate}}">{{cateTitle}}</router-link>
      </div>
      <div class="main">
        <div class="menu">
          <p style="color: #74777a;">{{$t("cms.otherhelp")}}</p>
          <div class="titles">
            <div class="route" v-for="item in topList" @click="clickTitle(item.id)"
              :class="{active:item.id==article.id}">
              <span>{{item.title}}</span>
            </div>
          </div>
          <router-link :to="{path:'helplist',query:{cate:cate}}">{{$t("common.more")}}>></router-link>
        </div>
        <div class="content-wrap">
          ...
          <div class="content">
            ...
            <div class="show-qrcode"
              style="text-align: right;padding: 15px 10px;border-radius:10px;background:#FFF;margin-top: 30px;">
      ...
      init() {
        Promise.all([this.getTop(), this.getArticle()]).then(arr => {
          if (
            ...
            window.document.title = (this.lang == "简体中文" ? "帮助 - " : "Help - ") + this.article.title + " - BitNexc | 全球比特币交易平台 | 全球数字货币交易平台";
          } else {
