<template>
	<div>
		<div class="container swap" v-if="0" :class="skin">
			<div class="main">
					...
					<div class="right d-none">
						<div class="coin-menu">
							<div style="padding: 8px 10px;height:48px;">
								<Input search :placeholder="$t('common.searchplaceholderswap')"
							...
							<Table height="463" @on-current-change="gohref" highlight-row id="USDT"
								v-show="basecion==='usdt'" :columns="coins.columns" :data="dataIndex"></Table>
							</Table>
						</div>
					</div>
					<div class="center">
						<div class="symbol d-none">
							...
							<div class="item">
								...
								<span class="num"
									:class="{buy:currentCoin.change>0,sell:currentCoin.change<0}">{{currentCoin.close |
									toFixed(4)}}</span>
						...
						<div class="imgtable d-none">
							<div class="handler">
								<span @click="changeImgTable('k')"
									:class="{active:currentImgTable==='k'}">{{$t("swap.kline")}}</span>
								<span @click="changeImgTable('s')"
									:class="{active:currentImgTable==='s'}">{{$t("swap.depth")}}</span>
							</div>
						...
						<div class="order" style="background: #999;margin-right: 5px;">
							<div class="order-handler">
								<span @click="changeOrder('currentPositions')"
									:class="{active:selectedOrder==='currentPositions'}">{{$t('swap.currentposition')}}</span>
								<span @click="changeOrder('currentEntrustOrders')"
									:class="{active:selectedOrder==='currentEntrustOrders'}">{{$t('swap.curdelegation')}}</span>
								<span @click="changeOrder('historyEntrustOrders')"
									:class="{active:selectedOrder==='historyEntrustOrders'}">{{$t('swap.hisdelegation')}}</span>
							</div>
							<div class="table">
								<Table height="285" v-if="selectedOrder==='currentPositions'"
									:columns="currentPosition.columns" :data="positionList"
									:no-data-text="$t('common.nodata')"></Table>
								<Table height="285" v-if="selectedOrder==='currentEntrustOrders'"
									:columns="currentOrder.columns" :no-data-text="$t('common.nodata')"
									:data="currentEntrustOrderList"></Table>
								<Table height="285" v-if="selectedOrder==='historyEntrustOrders'"
									:columns="historyOrder.columns" :no-data-text="$t('common.nodata')"
									:data="historyEntrustOrderList"></Table>
							</div>
						</div>
					</div>
				</div>
				<!-- 盘口 -->
				<div class="left plate-wrap d-none" style="position:relative; flex: 0 0 17%;">
					<div class="handlers">
					...
					<div class="plate-nowprice">
						<span class="price"
							:class="{buy:currentCoin.change>0,sell:currentCoin.change<0}">{{currentCoin.price |
							toFixed(baseCoinScale)}}</span>
					...
					<div class="order" style="margin-top: 5px;background-color: #192330;height: 320px;">
						<div class="order-handler" style="border-bottom: 1px solid rgb(39, 49, 62);">
							<span style="width: 50%;text-align: center;" @click="entrustChange(1)"
								:class="{active:entrustType===1}">{{$t('swap.open')}}</span>
							<span style="width: 50%;text-align: center;" @click="entrustChange(2)"
								:class="{active:entrustType===2}">{{$t('swap.close')}}</span>
						</div>
						<div class="table open-close">
							<div v-if="entrustType===1" class="open">
								<RadioGroup v-model="entrustOrderType" type="button" size="default"
								...
								<Form style="width: 94%;margin-left: 3%;margin-top:10px;">
									...
									<FormItem style="margin-bottom: 10px;" v-if="entrustOrderType=='0'">
										<label class="before">{{$t('swap.entrustprice')}}</label>
										<Input :value="$t('swap.marketpriceplaceholder')" disabled
									...
									<FormItem style="margin-bottom: 10px;">
										...
										<Input v-model="form.volume" v-if="sliderOpenPercent == 0"
										...
										<label class="after"
											v-if="sliderOpenPercent == 0">{{$t('swap.shareNumber')}}</label>
										<span
											style="position:absolute;top:32px;right:0px;color: #7c7f82;">1{{$t('swap.shareNumber')}}
											= {{contractCoinInfo.shareNumber}}USDT</span>
									</FormItem>
									<div class="split-panel" v-if="entrustOrderType!='2'">
										<div :class="{selected:Number(sliderOpenPercent)===25}"
											@click="onChangeOpenPercent(25)">25%</div>
										<div :class="{selected:Number(sliderOpenPercent)===50}"
											@click="onChangeOpenPercent(50)">50%</div>
										<div :class="{selected:Number(sliderOpenPercent)===75}"
											@click="onChangeOpenPercent(75)">75%</div>
										<div :class="{selected:Number(sliderOpenPercent)===100}"
											@click="onChangeOpenPercent(100)">100%</div>
									</div>

								</Form>
								<div class="open-tips">
									<div><span class="green">{{$t('swap.canup')}}:</span><span
											class="num">{{avaOpenBuy}} {{$t('swap.shareNumber')}}</span></div>
									<div><span class="red">{{$t('swap.candown')}}:</span><span
											class="num">{{avaOpenSell}} {{$t('swap.shareNumber')}}</span></div>
								</div>
								<div class="operate" v-if="isLogin">
									<Button type="primary" shape="circle" class="open-up"
										@click="onOpen(0)">{{$t('swap.openup')}}</Button>
									<Button type="primary" shape="circle" class="open-down"
										@click="onOpen(1)">{{$t('swap.opendown')}}</Button>
								</div>
							...
							<div v-else class="open">
								...
								<Form style="width: 94%;margin-left: 3%;margin-top:10px;">
									...
									<div class="split-panel" v-if="entrustOrderType!='2'">
										<div :class="{selected:Number(sliderClosePercent)===25}"
											@click="onChangeClosePercent(25)">25%</div>
										<div :class="{selected:Number(sliderClosePercent)===50}"
											@click="onChangeClosePercent(50)">50%</div>
										<div :class="{selected:Number(sliderClosePercent)===75}"
											@click="onChangeClosePercent(75)">75%</div>
										<div :class="{selected:Number(sliderClosePercent)===100}"
											@click="onChangeClosePercent(100)">100%</div>
									</div>
								</Form>
								<div class="open-tips">
									<div><span class="red">{{$t('swap.canclosedown')}}:</span><span
											class="num">{{contractWalletInfo.usdtBuyPosition}}
											{{$t('swap.shareNumber')}}</span></div>
									<div><span class="green">{{$t('swap.cancloseup')}}:</span><span
											class="num">{{contractWalletInfo.usdtSellPosition}}
											{{$t('swap.shareNumber')}}</span></div>
								</div>
								<div class="operate" v-if="isLogin">
									<Button type="primary" shape="circle" class="open-down"
										@click="onClose(1)">{{$t('swap.closedown')}}</Button>
									<Button type="primary" shape="circle" class="open-up"
										@click="onClose(0)">{{$t('swap.closeup')}}</Button>
								</div>
								<div class="operate-login" v-if="!isLogin" style="width: 94%; margin-left:3%;">
									<span
										style="display: inline-block;width: 100%; text-align: center;border:1px solid #232d3a;padding: 5px 0;border-radius: 5px;">
				...
				<!-- 成交记录 -->
				<div class="left plate-wrap d-none" style="position:relative; flex: 0 0 13%;">
					<div
					...
					<div class="trade-wrap">
						<Table height="472" :columns="trade.columns" :data="trade.rows"
							:no-data-text="$t('common.nodata')"></Table>
					</div>
					<!-- 我的合约账户 -->
					<div class="order"
						style="margin-top: 5px;min-height: 320px;background-color: #192330;color:#61688A;">
						<div
							...
							<router-link class="linkmore" to="/uc/swapAssets/">{{$t("swap.zijinhuazhuan")}}
							</router-link>
						</div>
						<div class="table swap-my-account">
							<div class="account-item" style="margin-top: 20px;">
								...
								<div class="mode" style="width: 65%;">
									<Button @click="showMarginModeModal()" v-if="marginMode === '1'"
										size="small">{{$t("swap.marginMode1")}}</Button>
									<Button @click="showMarginModeModal()" v-if="marginMode === '0'"
										size="small">{{$t("swap.marginMode2")}}</Button>
								</div>
							</div>
							<div class="account-item"
								...
								<div class="margin" style="width: 65%;display: flex;justify-content: space-between;">
									<Button @click="showAdjustLeverage(1)" size="small"
										style="flex: 0 0 47%;border:none;background: #4db872;">{{buyLeverage}}X{{$t('swap.up')}}</Button>
									<Button @click="showAdjustLeverage(2)" size="small"
										style="flex: 0 0 47%;border:none;background: #ee6560;">{{sellLeverage}}X{{$t('swap.down')}}</Button>
								</div>
							</div>

							<div class="account-item" style="margin-top:10px;">
								<div>{{$t("swap.accountquanyi")}}</div>
								<div><span>{{totalQuanyi | fixed4}}</span></div>
							</div>
							<div class="account-item">
								<div>{{$t("swap.profitandloss")}}</div>
								<div><span>{{profitAndLoss | fixed4}}</span></div>
							</div>
							<div class="account-item">
							...
							<div class="account-item">
								...
								<div><span>{{contractWalletInfo.usdtBuyPrincipalAmount +
										contractWalletInfo.usdtSellPrincipalAmount | fixed4}}</span></div>
							</div>
							<div class="account-item">
								<div>{{$t("swap.frozenAmount")}}</div>
								<div><span>{{contractWalletInfo.usdtFrozenBalance | fixed4}}</span></div>
							</div>
							<div class="account-item">
								<div>{{$t("swap.principalRate")}}</div>
								<div><span>{{assetRate | percent}} %</span></div>
							</div>
						</div>
					</div>
				</div>
			...
			<Modal v-model="marginModeModal" :title="$t('swap.modifyMarginModeTitle')" width="20"
				...
				<div style="text-align: center;width:100%;">
					<ButtonGroup style="width: 80%;text-align: center;">
						<Button style="width: 50%;" v-if="marginMode==='1'"
							type="primary">{{$t("swap.marginMode1")}}</Button>
						<Button style="width: 50%;" v-else
							@click="changeMarginMode('1')">{{$t("swap.marginMode1")}}</Button>

						<Button style="width: 50%;" v-if="marginMode==='0'"
							type="primary">{{$t("swap.marginMode2")}}</Button>
						<Button style="width: 50%;" v-else
							@click="changeMarginMode('0')">{{$t("swap.marginMode2")}}</Button>
					</ButtonGroup>
				</div>
				<div slot="footer">
			...
			<!-- 弹出框: 调整杠杆 -->
			<Modal v-model="leverageModal" :title="$t('swap.modifyLeverage')" width="25"
				class-name="vertical-center-modal">
				<div class="leverage">
					<!-- 连续杠杆 -->
					<InputNumber size="large" style="width: 100%;" :value.sync="leverageAdjustVal"
						v-if="leverageType==2" @on-change="onLeverageChange">
					</InputNumber>
					<!-- 离散杠杆 -->
					<div class="leverage-panel" v-if="leverageType==1 && changeLeverageType==1">
						<Button v-for="item in leverageList"
							style="margin-left:5px;margin-right:5px;margin-bottom: 10px;"
							@click="changeLeverage(Number(item))"
							:class="{leverageSelect:Number(item)===leverageAdjustVal}">{{item}}X</Button>
					</div>
		...
		<template v-if="!isHistoryOpen">
			...
			<div class="ws-exchange-head d-flex justify-content-between  p-2 mb-2">
				<div class="d-flex align-items-center">
					<span @click="isSidebarOpen = true">
						<b-icon icon="menu-button-wide" variant="light"></b-icon>
					</span>
					<div class="ps-2">{{ currentCoin.coin }}/{{ currentCoin.base }} Futures</div>
				</div>
			...
			<div class="swap pt-0">
				<div class="row mx-0 w-100">
					...
					<div class="col-7 px-0">
						<div class="order  h-100" style="margin-top: 5px;background-color: #192330;">

							<div class="table open-close">
								<div class="ws-sell-buy-tabs">
									<span style="font-size: 13px !important;"
										v-bind:class="[isLongOpen ? 'active buys' : 'buys']"
										@click="isLongOpen = true">{{$t('appmain.Openlong')}}
										<div></div>
									</span>
									<span style="font-size: 13px !important;"
										v-bind:class="[!isLongOpen ? 'active selles' : 'selles']"
										@click="isLongOpen = false">{{$t('appmain.OpenShort')}} <div></div></span>
								</div>
								<div v-if="entrustType===1" class="open pt-0">
									<div class="p-2 pb-0">
										<select v-model="entrustOrderType" @on-change="changeEntrustOrderType"
											class="ws-select">
											<option value="1">{{$t('swap.limited_price')}}</option>
											<option value="0">{{$t('swap.market_price')}}</option>
											<option value="2">{{$t('swap.spot_price')}}</option>
										</select>
									</div>
									<Form style="width: 94%;margin-left: 3%;margin-top:10px;">
										<FormItem style="margin-bottom: 10px;" v-if="entrustOrderType=='1'">
									...
									<div class="open-tips">
										<div style="flex:0 0 100% !important;" v-if="isLongOpen"><span
												class="green">{{$t('swap.canup')}}:</span><span
												class="num">{{avaOpenBuy}}
												{{$t('swap.shareNumber')}}</span></div>
										<div style="flex:0 0 100% !important;" v-if="!isLongOpen"><span
												class="red">{{$t('swap.candown')}}:</span><span
												class="num">{{avaOpenSell}}
												{{$t('swap.shareNumber')}}</span></div>
									</div>
									<div class="operate" v-if="isLogin">
										<Button v-if="isLongOpen" type="primary" shape="circle"
											class="open-up text-uppercase w-100"
											@click="onOpen(0)">{{$t('swap.openup')}}</Button>
										<Button v-if="!isLongOpen" type="primary" shape="circle"
											class="open-down text-uppercase w-100"
											@click="onOpen(1)">{{$t('swap.opendown')}}</Button>
									</div>
									<div class="operate-login" v-if="!isLogin" style="width: 94%; margin-left:3%;">
										<span
								...
								<div v-else class="open">
									...
									<div class="open-tips">
										<div style="flex:0 0 100% !important;" v-if="!isLongOpen"><span
												class="red">{{$t('swap.canclosedown')}}:</span><span
												class="num">{{contractWalletInfo.usdtBuyPosition}}
												{{$t('swap.shareNumber')}}</span></div>
										<div style="flex:0 0 100% !important;" v-if="isLongOpen"><span
												class="green">{{$t('swap.cancloseup')}}:</span><span
												class="num">{{contractWalletInfo.usdtSellPosition}}
												{{$t('swap.shareNumber')}}</span></div>
									</div>
									<div class="operate" v-if="isLogin">
										<Button v-if="!isLongOpen" type="primary" shape="circle"
											class="open-down text-uppercase"
											@click="onClose(1)">{{$t('swap.closedown')}}</Button>
										<Button v-if="isLongOpen" type="primary" shape="circle"
											class="open-up text-uppercase"
											@click="onClose(0)">{{$t('swap.closeup')}}</Button>
									</div>
									<div class="operate-login" v-if="!isLogin" style="width: 94%; margin-left:3%;">
										<span
					...
					<div class="col-5 px-0">
						<div class="h-100 ws-trade-inner-div-2 col px-1">
							<div>
								...
								<div class="row ws-ex-tr py-2 w-100 mx-0 border-0"
									v-for=" (item, index) in plate.askRows" v-if="index < 5">
								...
								<div class="row ws-ex-tr py-2 w-100 mx-0 border-0"
									v-for=" (item, index) in plate.bidRows" v-if="index < 5">
				...
				<div class="pt-2">
					<div class="order " style="background: #999;">
						<div class="order-handler d-flex" style="border-bottom: 1px solid rgb(39, 49, 62);">
							<span @click="changeOrder('currentPositions')"
								:class="{active:selectedOrder==='currentPositions'}"
								style="text-transform: capitalize !important">{{$t('appmain.OpenOrders')}} </span>
							<span @click="changeOrder('currentEntrustOrders')"
								:class="{active:selectedOrder==='currentEntrustOrders'}"
								style="text-transform: capitalize !important">{{$t('appmain.Position')}}</span>
							<span @click="isHistoryOpen = true" class="col text-warning"
								style="text-transform: capitalize !important; text-align:end !important; padding-right:15px !important;">{{$t('appmain.All')}}</span>
							<!-- <span @click="changeOrder('historyEntrustOrders')"
							:class="{active:selectedOrder==='historyEntrustOrders'}"
							style="text-transform: capitalize !important">{{$t('swap.hisdelegation')}}</span> -->
						</div>
						<div class="table">
							<Table height="285" v-if="selectedOrder==='currentPositions'"
								:columns="currentPosition.columns" :data="positionList"
		...
		<template v-if="isHistoryOpen && step == 0">
			<div class="pb-5">
				<div class="d-flex justify-content-between align-items-center ws-filter-tab "
					...
					<span @click="isHistoryOpen = false">
						<b-icon icon="chevron-left" variant="light"></b-icon>
					</span>
					<div class="d-flex col justify-content-center align-items-center">
						<span class="text-warning py-2" style="border-bottom: 1px solid orange;">{{$t('appmain.TradeHistory')}}</span>
					</div>
				</div>
				<div v-for="items in historyEntrustOrderList">
					<div @click="setDetail(items)" class=' p-2 d-flex flex-column'
						style="background-color:#1A212B; border-bottom: 1px solid #0E151F;">
						<div class="d-flex align-items-center justify-content-between">
							<div class="d-flex align-items-center col">
								<div v-if="items.direction=='SELL' && items.entrustType == 'CLOSE' "
									class=" fs-7 text-danger">Sell more</div>
								<div v-else-if="items.direction=='SELL' && items.entrustType == 'OPEN' "
									class=" fs-7 text-danger">Open short</div>
								<div v-else-if="items.direction=='BUY' && items.entrustType == 'OPEN' "
									class=" fs-7 text-success">Open long</div>
								<div v-else-if="items.direction=='BUY' && items.entrustType == 'CLOSE' "
									class=" fs-7 text-success">Buy short</div>
								<div class=" fs-8 ps-2" style="opacity: .7;">{{items.createTime}}</div>
							</div>
							<span>
						...
						<div class="row mx-0 ">
							<div class="col-3 px-0 d-flex flex-column align-items-start">
								<div class=" pt-1 fs-8 " style="opacity: .5;">{{$t('appmain.Amount')}}</div>
								<div class=" pt-1 fs-8 " style="opacity: .5;">{{items.volume}}{{$t('appmain.contract')}}</div>
							</div>
							<div class="col-3 px-0 d-flex flex-column align-items-center">
								<div class=" pt-1 fs-8 " style="opacity: .5;">{{$t('appmain.Price')}}</div>
								<div class=" pt-1 fs-8 " style="opacity: .5;">{{items.entrustPrice.toFixed(2)}}</div>
							</div>
							<div class="col-3 px-0 d-flex flex-column align-items-center">
								<div class=" pt-1 fs-8 " style="opacity: .5;">{{$t('appmain.Price')}}</div>
								<div class=" pt-1 fs-8 " style="opacity: .5;">{{items.tradedPrice.toFixed(2)}}</div>
							</div>
							<div class="col-3 px-0 d-flex flex-column align-items-end">
								<div class=" pt-1 fs-8 " style="opacity: .5;">{{$t('appmain.Closing')}} PnL</div>
								<div v-if="items.profitAndLoss < 0" class=" pt-1 fs-8  text-danger">
									{{items.profitAndLoss.toFixed(2)}}</div>
								<div v-if="items.profitAndLoss >= 0" style="opacity: .5;" class=" pt-1 fs-8 ">-</div>
							</div>
						</div>
					</div>
				</div>
		...
		<template v-if="step == 1">
			<div class="d-flex mb-2 justify-content-between align-items-center ws-filter-tab "
				style="padding-top: 3px !important; padding-bottom:3px !important;">
				<span @click="step = 0">
					<b-icon icon="chevron-left" variant="light"></b-icon>
				</span>
				<div class="d-flex col justify-content-center align-items-center">
					<span class=" py-2"> {{$t('appmain.Detail')}}</span>
				</div>
			</div>

			<div>
				<div class="p-2 px-3 text-lowercase "
					style="background-color: #1A212B; border-bottom:1px solid #0E151F;">
					{{ detailData.status }}
				</div>
				<div class="p-2 px-3 " style="background-color: #1A212B; border-bottom:1px solid #0E151F;">
					<div class="d-flex justify-content-between py-1 ">
						<span class="fs-8" style="opacity: .6;"> {{$t('appmain.Time')}}</span>
						<span class="fs-8" style="opacity: .6;">{{detailData.createTimeFull}}</span>
					</div>
					<div class="d-flex justify-content-between py-1 ">
						<span class="fs-8" style="opacity: .6;"> {{$t('appmain.Opening')}}/ {{$t('appmain.Liquidation')}}</span>
						<span class="fs-8" v-if="detailData.entrustType == 'CLOSE' "
							style="opacity: .6;">{{$t('appmain.Liquidation')}}</span>
						<span class="fs-8" v-if="detailData.entrustType == 'OPEN' " style="opacity: .6;">{{$t('appmain.Opening')}}</span>
					</div>
					<div class="d-flex justify-content-between py-1 ">
						<span class="fs-8" style="opacity: .6;">Side</span>
						<span v-if="detailData.direction=='SELL' && detailData.entrustType == 'CLOSE' "
							class=" fs-8 text-danger">{{$t('appmain.Sellmore')}}</span>
						<span v-else-if="detailData.direction=='SELL' && detailData.entrustType == 'OPEN' "
							class=" fs-8 text-danger">{{$t('appmain.Openshort')}}</span>
						<span v-else-if="detailData.direction=='BUY' && detailData.entrustType == 'OPEN' "
							class=" fs-8 text-success">{{$t('appmain.Openlong')}}</span>
						<span v-else-if="detailData.direction=='BUY' && detailData.entrustType == 'CLOSE' "
							class=" fs-8 text-success">{{$t('appmain.Buyshort')}}</span>
					</div>
					<div class="d-flex justify-content-between py-1 ">
						<span class="fs-8" style="opacity: .6;">Order Type</span>
						<span class="fs-8" v-if="detailData.type == 'MARKET_PRICE' " style="opacity: .6;">{{$t('appmain.Market')}}</span>
						<span class="fs-8" v-else style="opacity: .6;">-</span>
					</div>
					<div class="d-flex justify-content-between py-1 ">
						<span class="fs-8" style="opacity: .6;">Price</span>
						<span class="fs-8" style="opacity: .6;">{{detailData.triggerPrice == 0 ? '--' :
							detailData.triggerPrice }}</span>
					</div>
					<div class="d-flex justify-content-between py-1 ">
						<span class="fs-8" style="opacity: .6;">{{$t('appmain.Price')}}</span>
						<span class="fs-8" style="opacity: .6;">{{detailData.entrustPrice.toFixed(2)}}</span>
					</div>
					<div class="d-flex justify-content-between py-1 ">
						<span class="fs-8" style="opacity: .6;">{{$t('appmain.Price')}}</span>
						<span class="fs-8" style="opacity: .6;">{{detailData.tradedPrice.toFixed(2)}}</span>
					</div>
					<div class="d-flex justify-content-between py-1 ">
						<span class="fs-8" style="opacity: .6;">{{$t('appmain.Margin')}}</span>
						<span class="fs-8"
							style="opacity: .6;">{{detailData.principalAmount.toFixed(2)}}{{detailData.principalUnit}}</span>
					</div>
					<div class="d-flex justify-content-between py-1 ">
						<span class="fs-8" style="opacity: .6;">{{$t('appmain.Amount')}}</span>
						<span class="fs-8" style="opacity: .6;">{{detailData.volume}}{{$t('appmain.contract')}}</span>
					</div>
					<div class="d-flex justify-content-between py-1 ">
						<span class="fs-8" style="opacity: .6;">{{$t('appmain.Fee')}}</span>
						<span class="fs-8" style="opacity: .6;">{{detailData.closeFee}}</span>
					</div>
					<div class="d-flex justify-content-between py-1 ">
						<span class="fs-8" style="opacity: .6;">{{$t('appmain.Closing')}} PnL</span>
						<span v-if="detailData.profitAndLoss < 0"
							class=" fs-8  text-danger">{{detailData.profitAndLoss.toFixed(2)}}</span>
						<span v-if="detailData.profitAndLoss >= 0" style="opacity: .5;" class="fs-8 ">-</span>
					</div>
...
<script>
	...
	import CoinSidebar from "../exchange/CoinSidebar";
	...
	export default {
		components: { expandRow, DepthGraph, CoinSidebar },
		data() {
			let self = this;
			return {
				step: 0,
			...
			getHistoryEntrustOrders() {
				...
				this.$http
					...
					.then(response => {
						...
						this.historyEntrustOrderList.forEach((element, index) => {
							function msrow(t) {
							...
							function msrow2(t) {
								...
								let getrowdate = dateObject.toLocaleString();
								getrowdate = getrowdate.split(',').join(' ')
								getrowdate = getrowdate.split('/').join('-')
								return getrowdate;
							}

							this.historyEntrustOrderList[index].createTimeFull = msrow2(element.createTime)
							this.historyEntrustOrderList[index].createTime = msrow(element.createTime)
						});

						console.log(this.historyEntrustOrderList);
					});
			},
		}
	};
